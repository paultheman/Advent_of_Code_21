...
void findIndex(const std::vector<std::vector<int>>& boards, const int val, std::vector<std::vector<bool>>& mirror)
{
    for (size_t i {0}; i < boards.size(); ++i)
    {
        auto it { std::find (boards[i].begin(), boards[i].end(), val)};
        if (it != boards[i].end())
        {
            const int index { it - boards[i].begin()};
            // std::cout << "Board no. " << i << " Element "<< index << '\n';
            mirror.at(i).at(index) = 1;
        }
    }
}
...
int checkColumn (const std::vector<std::vector<bool>>& mirror)
{
    for (auto boards_it {mirror.begin()}; boards_it < mirror.end(); ++boards_it)
    {
        for (size_t i {}; i < 5; ++i)
        {
            ...
            if (occurances == 5)
            {
                std::cout << "Winning board no " << boards_it-mirror.begin() << " column " << i << '\n';
                return boards_it-mirror.begin();
            }
        }
    }
...
const std::vector<std::vector<int>> boards { createBoards() };
